...
function Set-PrimaryDisplayViaSDC {
    # SetDisplayConfig -- works over RDP without desktop context or GDI device names.
    # Works directly with adapter IDs from the Windows display configuration database.
    ...
    try {
        ...
        Write-Log "SDC: QueryDisplayConfig exception: $_" -Level "WARN"
        return $false
    }

    # Find source mode index for our target adapter
    $targetSrcIdx = $null

    # Try adapter LUID match first (most reliable)
    if ($Target.AdapterId) {
        for ($p = 0; $p -lt $numPaths; $p++) {
            if ($paths[$p].sourceInfo.adapterId.LowPart -eq $Target.AdapterId.LowPart -and
                $paths[$p].sourceInfo.adapterId.HighPart -eq $Target.AdapterId.HighPart) {
                $si = $paths[$p].sourceInfo.modeInfoIdx
                if ($si -ne 0xFFFFFFFF -and $si -lt $numModes) {
                    $targetSrcIdx = $si
                    Write-Log "SDC: Matched by LUID path[$p] srcIdx=$si"
                    break
                }
            }
        }
    ...
    # Fallback: match by target friendly name via re-query
    if ($null -eq $targetSrcIdx) {
        for ($p = 0; $p -lt $numPaths; $p++) {
            $tReq = New-Object DisplayAPI+DISPLAYCONFIG_TARGET_DEVICE_NAME
            $tReq.header.type      = [DisplayAPI]::DISPLAYCONFIG_DEVICE_INFO_GET_TARGET_NAME
            $tReq.header.size      = [System.Runtime.InteropServices.Marshal]::SizeOf($tReq)
            $tReq.header.adapterId = $paths[$p].targetInfo.adapterId
            $tReq.header.id        = $paths[$p].targetInfo.id
            [DisplayAPI]::DisplayConfigGetDeviceInfo([ref]$tReq) | Out-Null
            $fname = $tReq.monitorFriendlyDeviceName.Trim()
            $adapterReq = New-Object DisplayAPI+DISPLAYCONFIG_ADAPTER_NAME
            $adapterReq.header.type      = [DisplayAPI]::DISPLAYCONFIG_DEVICE_INFO_GET_ADAPTER_NAME
            $adapterReq.header.size      = [System.Runtime.InteropServices.Marshal]::SizeOf($adapterReq)
            $adapterReq.header.adapterId = $paths[$p].sourceInfo.adapterId
            $adapterReq.header.id        = $paths[$p].sourceInfo.id
            [DisplayAPI]::DisplayConfigGetDeviceInfo([ref]$adapterReq) | Out-Null
            $apath = $adapterReq.adapterDevicePath.Trim()
            Write-Log "SDC: path[$p] fname='$fname' adapter='$apath'"
            if (($fname -and $label -like "*$($fname.Split(' ')[0])*") -or
                ($apath -and $apath -like "*NVIDIA*" -and $label -like "*NVIDIA*")) {
                $si = $paths[$p].sourceInfo.modeInfoIdx
                if ($si -ne 0xFFFFFFFF -and $si -lt $numModes) {
                    $targetSrcIdx = $si
                    Write-Log "SDC: Matched by name path[$p] srcIdx=$si"
                    break
                }
            }
        }
    ...
    # Last resort: largest resolution source = most likely the GPU output
    if ($null -eq $targetSrcIdx) {
        Write-Log "SDC: No match, using largest source" -Level "WARN"
        $bestArea = 0
        for ($m = 0; $m -lt $numModes; $m++) {
            if ($modes[$m].infoType -eq 2) {
                $area = $modes[$m].info.sourceMode.width * $modes[$m].info.sourceMode.height
    ...
    if ($null -eq $targetSrcIdx) {
        Write-Log "SDC: Could not identify target source mode" -Level "WARN"
        return $false
    }

    # Shift all source positions so target lands at (0,0) -- that makes it primary
    $ox = $modes[$targetSrcIdx].info.sourceMode.position.x
    $oy = $modes[$targetSrcIdx].info.sourceMode.position.y
    Write-Log "SDC: Target srcIdx=$targetSrcIdx at ($ox,$oy) size=$($modes[$targetSrcIdx].info.sourceMode.width)x$($modes[$targetSrcIdx].info.sourceMode.height)"
    ...
    # Apply -- SDC_ALLOW_CHANGES lets Windows adjust mode if needed
    $flags = [DisplayAPI]::SDC_APPLY `
          -bor [DisplayAPI]::SDC_USE_SUPPLIED_DISPLAY_CONFIG `
          -bor [DisplayAPI]::SDC_SAVE_TO_DATABASE `
          -bor [DisplayAPI]::SDC_ALLOW_CHANGES
    try {
        $ret = [DisplayAPI]::SetDisplayConfig($numPaths, $paths, $numModes, $modes, $flags)
