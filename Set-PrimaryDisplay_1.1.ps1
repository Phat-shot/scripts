...
.DESCRIPTION
    ...
    3. WMI Win32_PnPEntity (finds NVIDIA Virtual Display and other PnP monitors)
    4. Registry HKLM\SYSTEM\CurrentControlSet\Control\Video (all driver-registered displays)
...
function Get-DisplaysViaQueryConfig {
    ...
    try {
        ...
        foreach ($path in $paths) {
            ...
            $width = 0u; $height = 0u; $hz = 0u; $posX = 0; $posY = 0
...
function Get-DisplaysViaWMI {
    ...
    try {
        $entities = Get-WmiObject Win32_PnPEntity -ErrorAction SilentlyContinue |
            Where-Object {
                $_.Name -match "NVIDIA Virtual|Virtual Display|Display Adapter|Video" -and
                $_.Name -notmatch "RDP|Remote Desktop|IDD"
            }
        ...
        Write-Log "WMI display detection failed: $_" -Level "WARN"
    }
    return $results
}
...
function Get-AllDisplays {
    ...
    # Start with QDC results (most complete over RDP)
    $merged = [System.Collections.Generic.List[PSCustomObject]]::new()

    foreach ($d in $qdc) { $merged.Add($d) }

    # Merge EnumDisplayDevices: update matching QDC entries with GDI device name and primary flag
    # or add if not already present
    foreach ($e in $enum) {
        $match = $merged | Where-Object { $_.DeviceName -eq $e.DeviceName -or
            ($_.FriendlyName -and $e.AdapterName -and $_.FriendlyName -like "*$($e.AdapterName.Split(' ')[0])*") } |
            Select-Object -First 1
        if ($match) {
            if (-not $match.DeviceName) { $match.DeviceName = $e.DeviceName }
            if ($e.IsPrimary)           { $match.IsPrimary  = $true }
            if ($e.Width -gt 0 -and $match.Width -eq 0) {
                $match.Width       = $e.Width
                $match.Height      = $e.Height
                $match.RefreshRate = $e.RefreshRate
                $match.PositionX   = $e.PositionX
                $match.PositionY   = $e.PositionY
            }
        } else {
            $merged.Add($e)
        }
    ...
    # Determine primary from current position (0,0) if not explicitly set
    ...
    # Add WMI items that aren't represented yet (e.g. NVIDIA Virtual Display not in QDC)
    foreach ($w in $wmi) {
        $alreadyPresent = $merged | Where-Object {
            $_.FriendlyName -like "*$($w.FriendlyName.Split(' ')[0])*" -or
            $_.AdapterName  -like "*$($w.FriendlyName.Split(' ')[0])*"
        }
        if (-not $alreadyPresent) { $merged.Add($w) }
    }

    # Number the final list
    $i = 0
    foreach ($d in $merged) {
...
# ─────────────────────────────────────────────────────────────
#  SET PRIMARY — via ChangeDisplaySettingsEx (needs active GDI name)
# ─────────────────────────────────────────────────────────────
function Set-PrimaryDisplayViaEnum {
    ...
    if (-not $activePair) {
        Write-Host "  Cannot set primary via display settings: no active GDI device names found." -ForegroundColor Red
        Write-Host "  This is expected in some RDP configurations. See note below." -ForegroundColor DarkGray
        return $false
    }
...
function Set-PrimaryDisplay {
    ...
    if ($ok) {
        ...
        Write-Host "  Could not apply change automatically via display API." -ForegroundColor Yellow
        Write-Host ""
        Write-Host "  To set the primary display manually over RDP:" -ForegroundColor White
        Write-Host "   1. Open: Settings -> System -> Display" -ForegroundColor DarkGray
        Write-Host "   2. Select the target display" -ForegroundColor DarkGray
        Write-Host "   3. Enable 'Make this my main display'" -ForegroundColor DarkGray
        Write-Host ""
        Write-Host "  Opening Display Settings now..." -ForegroundColor Yellow
        Start-Process "ms-settings:display" -ErrorAction SilentlyContinue
        Write-Log "Set-Primary via API failed for $label, opened ms-settings:display" -Level "WARN"
    }
}
