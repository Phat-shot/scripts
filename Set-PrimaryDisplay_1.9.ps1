...
# ─────────────────────────────────────────────────────────────
function Set-PrimaryDisplayViaSDC {
    # SetDisplayConfig -- works over RDP without GDI device names or desktop context.
    param([PSCustomObject]$Target)

    $label = if ($Target.FriendlyName) { $Target.FriendlyName } else { $Target.AdapterName }
    Write-Log "SDC attempt for '$label'"

    try {
        $numPaths = [uint32]0; $numModes = [uint32]0
        $ret = [DisplayAPI]::GetDisplayConfigBufferSizes(2, [ref]$numPaths, [ref]$numModes)
        if ($ret -ne 0 -or $numPaths -eq 0) {
            Write-Log "SDC: GetDisplayConfigBufferSizes ret=$ret paths=$numPaths" -Level "WARN"
            return $false
        }
        $paths = New-Object DisplayAPI+DISPLAYCONFIG_PATH_INFO[] $numPaths
        $modes = New-Object DisplayAPI+DISPLAYCONFIG_MODE_INFO[] $numModes
        $ret   = [DisplayAPI]::QueryDisplayConfig(2, [ref]$numPaths, $paths, [ref]$numModes, $modes, [IntPtr]::Zero)
        if ($ret -ne 0) { Write-Log "SDC: QueryDisplayConfig ret=$ret" -Level "WARN"; return $false }
        Write-Log "SDC: $numPaths active paths, $numModes modes"
    } catch {
        Write-Log "SDC: query exception: $_" -Level "WARN"; return $false
    }

    # ── Find target source mode index ─────────────────────────
    # Strategy A: match by adapter LUID stored in Target (set during Get-DisplaysViaQueryConfig)
    $targetSrcIdx = $null

    if ($Target.AdapterId -ne $null) {
        $tLow  = $Target.AdapterId.LowPart
        $tHigh = $Target.AdapterId.HighPart
        for ($p = 0; $p -lt $numPaths; $p++) {
            $sLow  = $paths[$p].sourceInfo.adapterId.LowPart
            $sHigh = $paths[$p].sourceInfo.adapterId.HighPart
            if ($sLow -eq $tLow -and $sHigh -eq $tHigh) {
                $si = $paths[$p].sourceInfo.modeInfoIdx
                if ($si -ne 0xFFFFFFFF -and [int]$si -lt $modes.Count) {
                    $targetSrcIdx = [int]$si
                    Write-Log "SDC: Matched by LUID ($tLow,$tHigh) -> path[$p] srcIdx=$targetSrcIdx"
                    break
                }
            }
        }
    ...
    # Strategy B: match by querying each path's target name (no struct assignment -- use index fields directly)
    if ($null -eq $targetSrcIdx) {
        Write-Log "SDC: No LUID match, trying target name query..."
        for ($p = 0; $p -lt $numPaths; $p++) {
            try {
                $tReq = New-Object DisplayAPI+DISPLAYCONFIG_TARGET_DEVICE_NAME
                $tReq.header.type           = [DisplayAPI]::DISPLAYCONFIG_DEVICE_INFO_GET_TARGET_NAME
                $tReq.header.size           = [System.Runtime.InteropServices.Marshal]::SizeOf($tReq)
                # Copy LUID fields individually -- PowerShell can't assign struct-to-struct
                $tReq.header.adapterId.LowPart  = $paths[$p].targetInfo.adapterId.LowPart
                $tReq.header.adapterId.HighPart = $paths[$p].targetInfo.adapterId.HighPart
                $tReq.header.id             = $paths[$p].targetInfo.id
                [DisplayAPI]::DisplayConfigGetDeviceInfo([ref]$tReq) | Out-Null
                $fname = $tReq.monitorFriendlyDeviceName.Trim()
                Write-Log "SDC: path[$p] targetName='$fname'"
                if ($fname -and ($label -like "*$($fname.Split(' ')[0])*" -or $fname -like "*$($label.Split(' ')[0])*")) {
                    $si = $paths[$p].sourceInfo.modeInfoIdx
                    if ($si -ne 0xFFFFFFFF -and [int]$si -lt $modes.Count) {
                        $targetSrcIdx = [int]$si
                        Write-Log "SDC: Matched by name '$fname' -> path[$p] srcIdx=$targetSrcIdx"
                        break
                    }
                }
            } catch { Write-Log "SDC: name query path[$p] error: $_" -Level "WARN" }
        }
    }

    # Strategy C: NVIDIA match -- if label contains NVIDIA, pick any path whose adapter
    # device path contains NVIDIA (query source name for GDI device, adapter name for path)
    if ($null -eq $targetSrcIdx -and $label -like "*NVIDIA*") {
        Write-Log "SDC: Trying NVIDIA adapter path match..."
        for ($p = 0; $p -lt $numPaths; $p++) {
            try {
                $sReq = New-Object DisplayAPI+DISPLAYCONFIG_SOURCE_DEVICE_NAME
                $sReq.header.type           = [DisplayAPI]::DISPLAYCONFIG_DEVICE_INFO_GET_SOURCE_NAME
                $sReq.header.size           = [System.Runtime.InteropServices.Marshal]::SizeOf($sReq)
                $sReq.header.adapterId.LowPart  = $paths[$p].sourceInfo.adapterId.LowPart
                $sReq.header.adapterId.HighPart = $paths[$p].sourceInfo.adapterId.HighPart
                $sReq.header.id             = $paths[$p].sourceInfo.id
                [DisplayAPI]::DisplayConfigGetDeviceInfo([ref]$sReq) | Out-Null
                $gdiName = $sReq.viewGdiDeviceName.Trim()
                Write-Log "SDC: path[$p] gdiName='$gdiName' adapterId=$($paths[$p].sourceInfo.adapterId.LowPart)"
            } catch { Write-Log "SDC: source name path[$p] error: $_" -Level "WARN" }
        }
        # Among active paths, pick the one with the largest source resolution (GPU output)
        $bestArea = 0
        for ($m = 0; $m -lt $modes.Count; $m++) {
            if ($modes[$m].infoType -eq 2) {
                $area = [long]$modes[$m].info.sourceMode.width * [long]$modes[$m].info.sourceMode.height
                if ($area -gt $bestArea) { $bestArea = $area; $targetSrcIdx = $m }
                Write-Log "SDC: srcMode[$m] $($modes[$m].info.sourceMode.width)x$($modes[$m].info.sourceMode.height) at ($($modes[$m].info.sourceMode.position.x),$($modes[$m].info.sourceMode.position.y))"
            }
        }
        if ($null -ne $targetSrcIdx) { Write-Log "SDC: Strategy C picked srcIdx=$targetSrcIdx (largest)" }
    }

    if ($null -eq $targetSrcIdx) {
        Write-Log "SDC: Could not identify target source mode -- giving up" -Level "WARN"
        return $false
    }

    # ── Shift positions so target lands at (0,0) ─────────────
    $ox = $modes[$targetSrcIdx].info.sourceMode.position.x
    $oy = $modes[$targetSrcIdx].info.sourceMode.position.y
    Write-Log "SDC: Shifting by (-$ox,-$oy). Target srcIdx=$targetSrcIdx was at ($ox,$oy)"

    for ($m = 0; $m -lt $modes.Count; $m++) {
        if ($modes[$m].infoType -eq 2) {
            $modes[$m].info.sourceMode.position.x -= $ox
            $modes[$m].info.sourceMode.position.y -= $oy
        }
    }

    # ── Apply ─────────────────────────────────────────────────
    $sdcFlags = [DisplayAPI]::SDC_APPLY `
             -bor [DisplayAPI]::SDC_USE_SUPPLIED_DISPLAY_CONFIG `
             -bor [DisplayAPI]::SDC_SAVE_TO_DATABASE `
             -bor [DisplayAPI]::SDC_ALLOW_CHANGES
    try {
        $ret = [DisplayAPI]::SetDisplayConfig($numPaths, $paths, $numModes, $modes, $sdcFlags)
        Write-Log "SDC: SetDisplayConfig ret=$ret (0=success)"
        return ($ret -eq 0)
    } catch {
        Write-Log "SDC: SetDisplayConfig exception: $_" -Level "WARN"
        return $false
    }
}
...
function Set-PrimaryDisplay {
    ...
    # SetDisplayConfig -- works over RDP, no GDI names or desktop context needed
    $ok = Set-PrimaryDisplayViaSDC -Target $Target

    if ($ok) {
        Write-Host "  Primary display updated successfully." -ForegroundColor Green
        Write-Log "Primary display set to: $label" -Level "OK"
    } else {
        ...
        Write-Log "Set-Primary failed for '$label'." -Level "ERROR"
    }
}
